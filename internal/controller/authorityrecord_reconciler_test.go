...
var _ = Describe("AuthorityRecordReconciler", func() {
	...
	It("applies AUTHREC for namelist profile when the connection is Ready", func() {
		const nlistProfile = "APP.NLIST"
		...
		_, err := rec.Reconcile(ctx, reconcile.Request{
			NamespacedName: types.NamespacedName{Namespace: ns, Name: "app-nlist-chg-dsp"},
		})
		Expect(err).NotTo(HaveOccurred())
