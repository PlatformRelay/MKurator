...
func TestAdoptionBlockForExisting_FailIfExists(t *testing.T) {
	...
	if block == nil || block.Reason != messagingv1beta1.ReasonAlreadyExists {
...
func TestAdoptionBlockForExisting_AdoptIfMatchingConflict(t *testing.T) {
	t.Parallel()
	block := adoptionBlockForExisting(
		messagingv1beta1.AdoptionPolicyAdoptIfMatching,
		true,
		true,
		true,
		`queue "APP.Q"`,
	...
	if block == nil || block.Reason != messagingv1beta1.ReasonAdoptionConflict {
		t.Fatalf("block = %#v", block)
	}
}
...
func TestHandleAdoptionBlock_NilBlock(t *testing.T) {
	...
	result, err := handleAdoptionBlock(
		context.Background(), nil, nil, &messagingv1beta1.Queue{}, 1, nil, syncStatusOpts{},
	)
...
func TestHandleAdoptionBlock(t *testing.T) {
	...
	q := &messagingv1beta1.Queue{ObjectMeta: metav1.ObjectMeta{Name: "orders", Namespace: "ns"}}
	cl := fake.NewClientBuilder().WithScheme(s).WithStatusSubresource(q).WithObjects(q).Build()
	block := &AdoptionBlockedError{Reason: messagingv1beta1.ReasonAlreadyExists, Message: "exists"}
...
func TestPatchSyncedAdoptionBlocked_AllKinds(t *testing.T) {
	...
	s := unitSchemeOrFatal(t)
	cases := []client.Object{
		&messagingv1beta1.Queue{ObjectMeta: metav1.ObjectMeta{Name: "q", Namespace: "ns"}},
		&messagingv1beta1.Topic{ObjectMeta: metav1.ObjectMeta{Name: "t", Namespace: "ns"}},
		&messagingv1beta1.Channel{ObjectMeta: metav1.ObjectMeta{Name: "c", Namespace: "ns"}},
		&messagingv1beta1.ChannelAuthRule{ObjectMeta: metav1.ObjectMeta{Name: "r", Namespace: "ns"}},
		&messagingv1beta1.AuthorityRecord{ObjectMeta: metav1.ObjectMeta{Name: "a", Namespace: "ns"}},
	}
	...
	for _, obj := range cases {
		if err := patchSyncedAdoptionBlocked(
			ctx, cl.Status(), nil, obj, 1, messagingv1beta1.ReasonAlreadyExists, "exists", syncStatusOpts{},
		); err != nil {
