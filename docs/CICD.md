...
Reusable composite actions under [`.github/actions/`](https://github.com/platformrelay/MKurator/tree/main/.github/actions) keep
...
|--------|---------------------|-------|---------|
| [`go-cache`](https://github.com/platformrelay/MKurator/tree/main/.github/actions/go-cache) | `${{ runner.os }}-go-${{ hashFiles('**/go.sum') }}` | `~/.cache/go-build`, `~/go/pkg/mod`; optional envtest: `~/.local/share/kubebuilder-envtest` keyed by `go.mod`+`go.sum` | `ci`, `preflight`, `release-gate`, `integration`, `e2e`, `nightly` |
| [`tools-bin`](https://github.com/platformrelay/MKurator/tree/main/.github/actions/tools-bin) | `${{ runner.os }}-tools-${{ hashFiles('Taskfile.yml', 'hack/install-external-tool.sh') }}` | `bin/kind`, `bin/mkcert`, `bin/terraform` | `e2e`, `nightly` (e2e jobs) |
| [`mq-docker-image`](https://github.com/platformrelay/MKurator/tree/main/.github/actions/mq-docker-image) | `${{ runner.os }}-ibm-mq-${{ hashFiles('hack/mq-docker/docker-compose.yml', 'hack/kind-cluster/terraform/variables.tf') }}` | `/tmp/ibm-mq-image.tar` (`docker save`/`load` of `icr.io/ibm-messaging/mq:…`) | `integration`, `e2e`, `nightly` |
| [`helm-cache`](https://github.com/platformrelay/MKurator/tree/main/.github/actions/helm-cache) | `${{ runner.os }}-helm-${{ hashFiles('hack/kind-cluster/terraform/variables.tf') }}` | `~/.cache/helm` | `e2e`, `nightly` (e2e jobs) |

Image reference for MQ caching is resolved by [`hack/ci/mq-image-ref.sh`](https://github.com/platformrelay/MKurator/blob/main/hack/ci/mq-image-ref.sh)
(must match `hack/mq-docker/docker-compose.yml`). E2e platform bring-up uses
[`hack/ci/cluster-up-with-mq-image.sh`](https://github.com/platformrelay/MKurator/blob/main/hack/ci/cluster-up-with-mq-image.sh):
...
(`go tool cover -func`), and publishes to [Codecov](https://codecov.io/gh/platformrelay/MKurator)
(`codecov.yml`) via `codecov/codecov-action` with GitHub OIDC (`use_oidc: true`;
`fail_ci_if_error: false` so fork PRs without OIDC do not fail the job;
`codecov.yml` uses `target: auto` only — no strict coverage gate in CI).
Install the [Codecov GitHub App](https://github.com/apps/codecov) on the repo once
for reliable PR patch comments. Coverage regressions are investigated, not ignored.
...
then [`hack/helm-verify-admission.sh`](https://github.com/platformrelay/MKurator/blob/main/hack/helm-verify-admission.sh) and
...
[`mq-docker-image`](https://github.com/platformrelay/MKurator/tree/main/.github/actions/mq-docker-image) → `task mq:integration:up` → `task mq:integration:wait` → `task test:integration`
...
### `e2e`
Dedicated workflow [`.github/workflows/e2e.yaml`](https://github.com/platformrelay/MKurator/blob/main/.github/workflows/e2e.yaml):
...
Dedicated workflow [`.github/workflows/nightly.yaml`](https://github.com/platformrelay/MKurator/blob/main/.github/workflows/nightly.yaml):
...
Dedicated workflow [`.github/workflows/release-gate.yaml`](https://github.com/platformrelay/MKurator/blob/main/.github/workflows/release-gate.yaml)
...
**`e2e (helm)`** is optional for tagging. Uses [`hack/ci/wait-release-gate-checks.sh`](https://github.com/platformrelay/MKurator/blob/main/hack/ci/wait-release-gate-checks.sh).
...
release assets via [`hack/release-assets.sh`](https://github.com/platformrelay/MKurator/blob/main/hack/release-assets.sh)
...
Install instructions are appended from [`.github/release-notes-install.md`](https://github.com/platformrelay/MKurator/blob/main/.github/release-notes-install.md)
via [`hack/assemble-release-notes.sh`](https://github.com/platformrelay/MKurator/blob/main/hack/assemble-release-notes.sh). Checkout
...
[`go-cache`](https://github.com/platformrelay/MKurator/tree/main/.github/actions/go-cache) action (see [Workflow caching](#workflow-caching)).
IBM MQ image reuse uses [`mq-docker-image`](https://github.com/platformrelay/MKurator/tree/main/.github/actions/mq-docker-image);
...
[`.github/workflows/renovate.yaml`](https://github.com/platformrelay/MKurator/blob/main/.github/workflows/renovate.yaml) using
...
| File | Role |
|------|------|
| [`.github/renovate-config.json`](https://github.com/platformrelay/MKurator/blob/main/.github/renovate-config.json) | **Global** (self-hosted) config passed to the action: target repo via `RENOVATE_REPOSITORIES`, onboarding disabled |
| [`renovate.json`](https://github.com/platformrelay/MKurator/blob/main/renovate.json) | **Repository** config: schedules, grouping, custom managers, package rules |
