...
| From | To | Highlights |
|------|-----|------------|
| **0.15.2** | **0.15.3** | Patch — dependency security maintenance. `google.golang.org/grpc` 1.82.1 → **1.83.2** (GHSA-vp52-pcj8-j9qc, GHSA-2v4p-qf9q-27wj) and the OpenTelemetry SDK/OTLP exporters 1.44.0 → **1.45.0** (GHSA-8wmf-6v46-5gfg), plus routine Go module and CI action updates. No API, CRD or behaviour change — pull the new image. No action required. |
