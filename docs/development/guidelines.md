...
- The mqrest `ClientFactory` re-reads the v1beta1 hub to resolve the union's effective credentials Secret, and the client cache fingerprint keys off **that** Secret's resourceVersion (the secret actually used) ([ADR-0023](../adr/0023-connection-client-cache-lifecycle.md)). **Rotation gap in this slice (AUTH-12):** the fingerprint only matters once a reconcile runs, and a `spec.authentication` union secret rotation is **not** watch-driven and has **no** periodic-resync backstop on the `QueueManagerConnection`. The Secret watch (`connectionReferencesSecret`) matches only the spoke's `credentialsSecretRef`/`caSecretRef`, and the QMC Ready path returns without a `RequeueAfter`. So rotating a union secret does **not** invalidate the cached client until the next reconcile trigger (spec change, TLS/CA-secret change, or operator restart) — union users are strictly worse than legacy Basic on rotation here. Closing this (extending `cacheFingerprint` **and** `connectionReferencesSecret` to union refs) is deferred to **AUTH-14**; do not claim rotation parity for the union in this slice.
