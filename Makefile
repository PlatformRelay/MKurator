...
KIND_CLUSTER ?= mkurator-test-e2e
...
docker-buildx: ## Build and push docker image for the manager for cross-platform support
	...
	- $(CONTAINER_TOOL) buildx create --name mkurator-builder
	$(CONTAINER_TOOL) buildx use mkurator-builder
	- $(CONTAINER_TOOL) buildx build --push --platform=$(PLATFORMS) --tag ${IMG} -f Dockerfile.cross .
	- $(CONTAINER_TOOL) buildx rm mkurator-builder
	rm Dockerfile.cross
...
$(LOCALBIN)/kustomize: $(LOCALBIN)
	$(call go-install-tool,$(LOCALBIN)/kustomize,sigs.k8s.io/kustomize/kustomize/v5,$(KUSTOMIZE_VERSION))
